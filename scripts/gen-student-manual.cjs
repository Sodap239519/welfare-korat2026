...
children.push(h3('ส่วนที่ 2 · วันที่ เวลา และหน่วยบริการ'));
['**วันที่ปฏิบัติงาน** (บังคับ) — ค่าเริ่มต้น = วันนี้', '**เวลาเริ่ม / เวลาสิ้นสุด** (ไม่บังคับ) — เป็นเวลาตามที่กรอก (เวลาไทย)', '**หน่วยบริการที่ปฏิบัติงาน** — ระบบแสดงให้อัตโนมัติจากข้อมูลตอนสมัคร (ที่ว่าการอำเภอ หรือ ธนาคาร+สาขา) ไม่ต้องกรอกเอง'].forEach(t => children.push(bullet(t)));
...
children.push(table(
  ['ช่อง', 'รายละเอียด'],
  [
    ['ช่วงเวลา', 'เช้า / บ่าย / เย็น — เลือก "เพิ่มเติม" ระบบตั้งเป็น "เย็น" ให้อัตโนมัติ (แก้เป็นช่วงอื่นได้) · เพิ่มกิจกรรมใหม่ตั้งค่าเริ่มต้นเป็น "เย็น"'],
    ['ประเภทกิจกรรม', 'เช่น ให้คำแนะนำการลงทะเบียน · ช่วยกรอกข้อมูลลงทะเบียน · ตรวจสอบเอกสาร · คัดกรองกลุ่มตกหล่น · จัดอบรม/ให้ความรู้'],
...
children.push(callout('ตัวเลขลงทะเบียน (ไม่บังคับให้ตรง)', [
  'ถ้ากิจกรรมเป็น **การช่วยลงทะเบียน** — ปกติ สำเร็จ + ไม่สำเร็จ ควรเท่ากับผู้รับบริการรวม (ถ้าไม่ตรงจะขึ้น **เตือนสีส้ม** แต่ยังบันทึกได้)',
  'ถ้ากิจกรรมเป็น **อบรม/อื่น ๆ** ที่ผู้รับบริการ = ผู้เข้าร่วม (ไม่ใช่ผู้ลงทะเบียน) — **ไม่ต้องใส่** ตัวเลขลงทะเบียนให้ตรงก็ได้ ระบบไม่บังคับ',
]));
...
children.push(spacer());
children.push(p('เมื่อครบแล้วกด **"บันทึก"** (สีน้ำเงิน) ระบบจะบันทึกและกลับสู่รายการ'));

children.push(spacer());
children.push(h2('ดูบันทึก และออกรายงานรายวัน (PDF) สำหรับแนบเบิกจ่าย'));
children.push(p('การ์ดของแต่ละวันแสดง **วันที่แบบไทย** (เช่น 7 มิถุนายน 2569) พร้อมสรุปแถวเดียว: ผู้รับบริการ / ลงทะเบียนสำเร็จ-ไม่สำเร็จ / กรณีปัญหา / ไฟล์แนบ และมีปุ่ม 3 ปุ่ม:'));
children.push(table(
  ['ปุ่ม', 'หน้าที่'],
  [
    ['👁 ดู', 'เปิดมุมมองอ่านอย่างเดียว (ไม่แก้ไข) — แสดง วันที่ · ช่วงเวลา · หน่วยบริการ · กิจกรรม · ผลลงทะเบียน · กรณีปัญหา · ภาพ · ผู้ควบคุมงาน'],
    ['✏️ แก้ไข', 'เปิดฟอร์มเพื่อแก้ไขบันทึกของวันนั้น'],
    ['🗑️ ลบ', 'ลบบันทึกของวันนั้น'],
  ],
  [2000, 7026],
));
children.push(spacer());
children.push(p('ในมุมมอง **ดู** กดปุ่ม 🖨️ **"พิมพ์ / บันทึก PDF"** → ระบบเปิดหน้ารายงานขนาด A4 ที่จัดรูปแบบเรียบร้อย จากนั้นในกล่องพิมพ์ของเบราว์เซอร์เลือกปลายทางเป็น **"บันทึกเป็น PDF (Save as PDF)"** เพื่อได้ไฟล์รายงาน'));
children.push(callout('ใช้แนบเอกสารเบิกจ่าย 🧾', [
  'รายงานรายวันมีครบ: หัวเรื่องโครงการ · ชื่อนักศึกษา · วันที่ (แบบไทย) · หน่วยบริการ · ตารางกิจกรรม · สรุปผลลงทะเบียน · ตารางกรณีปัญหา · ภาพการปฏิบัติงาน · ช่องลงชื่อผู้ควบคุมงาน',
  'พิมพ์ออกมาเซ็น หรือบันทึกเป็น PDF ไปแนบกับเอกสารเบิกจ่ายได้ทันที',
]));
